...
namespace Tests.EditMode.Utils
{
    ...
    /// </summary>
    [TestFixture]
    public class ColliderCastOverlappingTests
    {
        private GameObject go;
        private GameObject hitGo;
        private PrimitiveColliderCast primitiveColliderCast;
        ...
        [Test]
        public void ColliderCastHitOverlapping()
        {
            var colliderCastHit = this.rigidbodyColliderCast.CastSelf(Vector3.down, 1.0f);
            Assert.IsTrue(colliderCastHit.collider.gameObject == hitGo);
            Assert.IsTrue(colliderCastHit.distance == 0);
        }
    }
}